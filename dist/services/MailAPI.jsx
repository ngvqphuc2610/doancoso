...
export const handleContactForm = async (req, res) => {
    try {
        ...
        if (!process.env.EMAIL || !process.env.EMAIL_PASSWORD) {
            await pool.execute("UPDATE CONTACT SET status = 'email_failed' WHERE id_contact = ?", [result.insertId]);
            return res.status(500).json({
                error: "Lỗi cấu hình email máy chủ.",
                contactSaved: true,
                message: "Thông tin liên hệ đã được lưu nhưng không thể gửi email thông báo."
            });
        }
        try {
            ...
            // Cập nhật trạng thái trong database khi gửi email thất bại
            await pool.execute("UPDATE CONTACT SET status = 'email_failed' WHERE id_contact = ?", [result.insertId]);
            console.error("Lỗi gửi email:", emailError);
            res.status(500).json({
                success: false,
                error: "Không thể gửi email thông báo",
                details: emailError.message,
                contactSaved: true,
                message: "Thông tin liên hệ đã được lưu nhưng không thể gửi email thông báo. Chúng tôi sẽ liên hệ lại sau."
            });
        }
    }
    catch (error) {
...
export const getContacts = async (req, res) => {
    try {
        ...
        const [contacts] = await pool.execute(`SELECT c.*, s.name as staff_name 
             FROM CONTACT c 
             LEFT JOIN STAFF s ON c.id_staff = s.id_staff 
             ORDER BY c.contact_date DESC LIMIT ? OFFSET ?`, [limit, offset]);
        // Đếm tổng số liên hệ
        const [countResult] = await pool.execute("SELECT COUNT(*) as total FROM CONTACT");
...
export const getContactById = async (req, res) => {
    try {
        ...
        if (contacts[0].status === 'unread') {
            await pool.execute("UPDATE CONTACT SET status = 'read' WHERE id_contact = ? AND status = 'unread'", [contactId]);
        }
        res.status(200).json({
            success: true,
            contact: contacts[0]
...
export const replyContact = async (req, res) => {
    try {
        ...
        const [contacts] = await pool.execute("SELECT * FROM CONTACT WHERE id_contact = ?", [contactId]);
        if (contacts.length === 0) {
            return res.status(404).json({ error: "Không tìm thấy thông tin liên hệ." });
        }
        const contact = contacts[0];
        // Cập nhật trạng thái và phản hồi
        const [result] = await pool.execute("UPDATE CONTACT SET status = 'replied', reply = ?, reply_date = NOW(), id_staff = ? WHERE id_contact = ?", [reply, id_staff, contactId]);
...
export const deleteContact = async (req, res) => {
    try {
        const contactId = req.params.id;
        const [result] = await pool.execute("DELETE FROM CONTACT WHERE id_contact = ?", [contactId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: "Không tìm thấy thông tin liên hệ." });
        }
        res.status(200).json({
