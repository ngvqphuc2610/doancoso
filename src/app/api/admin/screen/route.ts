...
export async function GET(req: NextRequest) {
    try {
        console.log(`[screen API] Connecting to Express API at: ${API_URL}/api/admin/screen`);

        const response = await axios.get(`${API_URL}/api/admin/screen`, {
            timeout: 10000,
            headers: {
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!body.start_time) {
            return NextResponse.json(
                { success: false, message: 'vị trí phòng là bắt buộc' },
                { status: 400 }
            );
        }

        const response = await axios.post(`${API_URL}/api/admin/screen`, body, {
            timeout: 5000
        });
